 ...
 * @package Msls
 */
class N2GoMslsOutput extends MslsMain {
		...
		if ( $blogs ) {
			...
			foreach ( $blogs as $blog ) {
				...
				if ( $current ) {
					...
					if ( 'N2GoMslsOptions' != get_class( $mydata ) && $exists && ! $mydata->has_value( $language ) ) {
						/**
				...
				$link->src = N2GoMslsOptions::instance()->get_flag_url( $language );
				$link->alt = $language;
	...
	public function __toString() {
		$options = N2GoMslsOptions::instance();
