(function ($) {
	...
	window.N2GoPageTabAnchorsView = vc.shortcode_view.extend({
	 ...
	 addTab:function (e) {
	  ...
	  var tab_title = window.i18nLocale.tab,
	   tabs_count = this.$tabs.find('[data-element_type=n2go_page_tab_anchor]').length,
	   tab_id = (+new Date() + '-' + tabs_count + '-' + Math.floor(Math.random() * 11));
	  vc.shortcodes.create({shortcode:'n2go_page_tab_anchor', params:{title:tab_title, tab_id:tab_id}, parent_id:this.model.id});
	  return false;
	 },
	 ...
	 cloneModel:function (model, parent_id, save_order) {
	  ...
	  if (model.get('shortcode') === 'n2go_page_tab') _.extend(new_params, {tab_id:+new Date() + '-' + this.$tabs.find('[data-element-type=n2go_page_tab_anchor]').length + '-' + Math.floor(Math.random() * 11)});
	  model_clone = Shortcodes.create({shortcode:model.get('shortcode'), id:vc_guid(), parent_id:parent_id, order:new_order, cloned:(model.get('shortcode') === 'n2go_page_tab_anchor' ? false : true), cloned_from:model.toJSON(), params:new_params});
	...
	window.N2GoPageTabAnchorView = window.VcColumnView.extend({
	 events:{
	  'click > .vc_controls .vc_control-btn-delete':'deleteShortcode',
	 ...
	 cloneModel:function (model, parent_id, save_order) {
	  var shortcodes_to_resort = [],
	   new_order = _.isBoolean(save_order) && save_order === true ? model.get('order') : parseFloat(model.get('order')) + vc.clone_index,
	   new_params = _.extend({}, model.get('params'));
	  if (model.get('shortcode') === 'n2go_page_tab_anchor') _.extend(new_params, {tab_id:+new Date() + '-' + this.$tabs.find('[data-element_type=n2go_page_tab_anchor]').length + '-' + Math.floor(Math.random() * 11)});
